...
public class AccidentInsurance extends Insurance {
    ...
    public void showInfo() {
        ...
        System.out.println("- 피보험자 직업별 요율(위험군, 비위험군): " +
                this.getSpecificDataMap().get("occupation_rate_risk") + ", " +
                this.getSpecificDataMap().get("occupation_rate_non_risk"));
        System.out.println("* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - *");
        System.out.println("+ 일반상해후유장해 최대 보장금액: " +
                printComma(Integer.parseInt((String) this.getSpecificDataMap().get("guaranteed_general_accident_injury"))));
        System.out.println("- 일반상해사망 최대 보장금액: " +
                printComma(Integer.parseInt((String) this.getSpecificDataMap().get("guaranteed_general_accident_death"))));
        System.out.println("- 특수상해후유장해 최대 보장금액: " +
                printComma(Integer.parseInt((String) this.getSpecificDataMap().get("guaranteed_specific_accident_injury"))));
        System.out.println("- 특수상해사망 최대 보장금액: " +
                printComma(Integer.parseInt((String) this.getSpecificDataMap().get("guaranteed_specific_accident_death"))));
        System.out.println("*---------------------------------------------------------------------------*");
    }
}
